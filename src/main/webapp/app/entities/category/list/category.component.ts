...
export class CategoryComponent implements OnInit {
  ...
  totalItems = 0;
  page = 1;
  categoryStatusList: IPickListData[] = [];
  ...
  getCategoryStatus(value?: number | null): string | undefined {
    const categoryStatus = this.categoryStatusList?.find(item => item.value === value);
    return categoryStatus?.description;
  }
  ...
  load(): void {
    this.loadSetupService.loadPicklist().subscribe(res => {
      if (res.body) {
        this.categoryStatusList = res.body.catetoryStatusList;
      }
    });
