...
public class ReservationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reservations").get("/api/reservations").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new reservation")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"phone\": \"SAMPLE_TEXT\"" +
                                ", \"reservedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reservation").get("${new_reservation_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reservation").delete("${new_reservation_url}").headers(headers_http_authenticated))
                .pause(10)
        );
