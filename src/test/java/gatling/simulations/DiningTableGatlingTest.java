 ...
 */
public class DiningTableGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all diningTables").get("/api/dining-tables").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new diningTable")
                        .post("/api/dining-tables")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"tableNumber\": 0" + ", \"seatingCapacity\": 0" + ", \"tableStatus\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_diningTable_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created diningTable").get("${new_diningTable_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created diningTable").delete("${new_diningTable_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DiningTable entity").exec(scn);
