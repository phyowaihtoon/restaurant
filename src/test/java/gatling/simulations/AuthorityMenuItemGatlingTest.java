...
/**
 * Performance test for the AuthorityMenuItem entity.
 */
public class AuthorityMenuItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all authorityMenuItems")
                    .get("/api/authority-menu-items")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new authorityMenuItem")
                        .post("/api/authority-menu-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"isAllow\": 0" + ", \"isRead\": 0" + ", \"isWrite\": 0" + ", \"isDelete\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_authorityMenuItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created authorityMenuItem").get("${new_authorityMenuItem_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created authorityMenuItem").delete("${new_authorityMenuItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AuthorityMenuItem entity").exec(scn);
