...
/**
 * Performance test for the AddOnItem entity.
 */
public class AddOnItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all addOnItems").get("/api/add-on-items").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new addOnItem")
                        .post("/api/add-on-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"itemName\": \"SAMPLE_TEXT\"" + ", \"price\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_addOnItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created addOnItem").get("${new_addOnItem_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created addOnItem").delete("${new_addOnItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AddOnItem entity").exec(scn);
