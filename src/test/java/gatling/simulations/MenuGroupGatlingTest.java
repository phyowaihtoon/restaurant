...
/**
 * Performance test for the MenuGroup entity.
 */
public class MenuGroupGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all menuGroups").get("/api/menu-groups").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new menuGroup")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"isChild\": 0" +
                                ", \"parentId\": 0" +
                                ", \"translateKey\": \"SAMPLE_TEXT\"" +
                                ", \"orderNo\": 0" +
                                ", \"faIcon\": \"SAMPLE_TEXT\"" +
                                ", \"routerLink\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_menuGroup_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created menuGroup").get("${new_menuGroup_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created menuGroup").delete("${new_menuGroup_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MenuGroup entity").exec(scn);
