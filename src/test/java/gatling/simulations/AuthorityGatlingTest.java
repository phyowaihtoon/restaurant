 ...
 */
public class AuthorityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all authorities").get("/api/authorities").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new authority")
                        .post("/api/authorities")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"description\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_authority_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created authority").get("${new_authority_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created authority").delete("${new_authority_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Authority entity").exec(scn);
